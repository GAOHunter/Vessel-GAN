...
def discriminator_image(img_size, n_filters, init_lr, tensorboard, name='d'):
    ...
    conv1 = extractor(inputs, 2 * n_filters)
    conv1 = LeakyReLU(0.2)(conv1)
    conv2 = extractor(conv1, 2 * n_filters, s=1)
    conv2 = LeakyReLU(0.2)(conv2)
    conv3 = extractor(conv2, 2 * n_filters, s=1)
    conv3 = LeakyReLU(0.2)(conv3)
    conv4 = Concatenate(axis=3)([conv1, conv2, conv3])
    conv4 = Conv2D(2*n_filters, kernel_size=(3, 3), padding="same")(conv4)
